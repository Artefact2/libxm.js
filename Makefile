...
frontend/index.html: build/libxmjs.a build/libxm_build/libxm.a libxm.js build/shell.html
	emcc --pre-js libxm.js --shell-file build/shell.html -flto -Oz --closure 1 -sENVIRONMENT=web -sSINGLE_FILE=1 -sEXPORTED_FUNCTIONS="['_a', '_b', '_x', '_xm_mute_channel', '_xm_mute_instrument', '_xm_get_latest_trigger_of_instrument', '_xm_get_latest_trigger_of_channel', '_xm_is_channel_active', '_xm_get_instrument_of_channel', '_xm_get_frequency_of_channel', '_xm_get_volume_of_channel', '_xm_get_panning_of_channel']" -sEXPORTED_RUNTIME_METHODS=HEAPU8,HEAPF32 build/libxmjs.a build/libxm_build/libxm.a -o $@ || (rm -f $@; exit 1)
